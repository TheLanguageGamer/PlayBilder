...
class Board {
	...
	needsEdgeToUp(rule : Rule, i : number, j : number) {
		return this.hasRule(i, j-1, rule.index)
				&& !this.hasRule(i-1, j-1, rule.index)
				&& !rule.boundaryEdges.has(JSON.stringify([i, j, i, j-1]));
	}

	advanceUp(rule : Rule, i : number, j : number) {
	...
	needsEdgeToLeft(rule : Rule, i : number, j : number) {
		return this.hasRule(i-1, j-1, rule.index)
				&& !this.hasRule(i-1, j, rule.index)
				&& !rule.boundaryEdges.has(JSON.stringify([i, j, i-1, j]));
	}

	advanceLeft(rule : Rule, i : number, j : number) {
	...
	needsEdgeToDown(rule : Rule, i : number, j : number) {
		return this.hasRule(i-1, j, rule.index)
				&& !this.hasRule(i, j, rule.index)
				&& !rule.boundaryEdges.has(JSON.stringify([i, j, i, j+1]));
	}

	advanceDown(rule : Rule, i : number, j : number) {
	...
	advanceBoundary(direction : Direction, rule : Rule, i : number, j : number) {
	...
	visitRuleIndex(i : number, j : number, countIndex : number, maskIndex : number) {
		if (i < 0 || j < 0 || i >= this.grid.gridSize.width || j >= this.grid.gridSize.height) {
			return;
		}
	...
	canDrawRule(i : number, j : number) {
		return this.findAdjacentRule(i, j).count <= 1;
	}

	rulePad(i : number, j : number) {
	...
	undoCellEdit(cellEdit : CellEdit) {
	...
	undoRuleMove(ruleMove : RuleMove) {
	...
	respositionEdgesForRule(rule : Rule) {
		for (let edge of this.edges) {
	...
	isRuleGraphCyclic_VisitRule(rule : Rule, visited : boolean[], recStack : boolean[]) {
