...
class EditorTools {
	...
	selectedCoord = {i : 0, j : 0};
	setSelectedFromPalette : (i : number, j : number) => void;
	...
	constructor(
		...
		this.setSelectedFromPalette = function(i : number, j : number) {
	...
	screenDidResize(screenSize : Size, cp : ContentProvider, tileSize : number) {
			...
			container,
			{
		    	onKeyDown(e : KeyboardEvent) {
		    		...
		    		if (board.state == BoardState.Edit && _this.editorTools) {
			    		...
			    		if (!isNaN(asNumber)) {
			    			_this.editorTools.setSelectedFromPalette(
			    				_this.editorTools.selectedCoord.i,
			    				asNumber
			    			);
			    		} else if (e.key == "b") {
			    			_this.editorTools.setSelectedFromPalette(
			    				0,
			    				_this.editorTools.selectedCoord.j
			    			);
			    		} else if (e.key == "i") {
			    			_this.editorTools.setSelectedFromPalette(
			    				1,
...
window.addEventListener('beforeunload', function (e) {
    ...
    if ($playBilder.editorTools) {
	    let archive = JSON.stringify(
	    	$playBilder.board.save($playBilder.editorTools.gameSettingsGui.save()),
	    	null,
	    	'\t'
	    );
